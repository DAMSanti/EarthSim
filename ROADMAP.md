...
- Nota: simulación a ~10 pasos/segundo reportada por el usuario en `TectonicPlanetActor` — el bucle de steps no tiene límite propio, así que esto apunta a que el framerate real sigue siendo bajo en estado estable (no solo durante la carga inicial), probablemente porque Nanite Displacement re-teselado cada frame según la vista no es gratis ni con 6 parches. Mismo origen que la deuda arquitectónica de arriba, sin diagnosticar más a fondo todavía (no se puede perfilar en vivo desde aquí).
