...
- [x] **Bloqueador confirmado (no solo sospechado)**: `Content/` solo contiene `M_VertexColor.uasset` (usado por `TectonicsTestActor`, no por Nanite). `NaniteConfig.PlanetMaterial` (`NaniteTypes.h:75`) es un `TSoftObjectPtr` sin ningún valor por defecto en código. **El material WPO para el pipeline Nanite no existe en el proyecto** - no es un problema de conexión, nunca se creó. `GetPatchMaterial()` devuelve `nullptr` en tiempo de ejecución hoy.
- [ ] **Especificación para crear el material** (pendiente, requiere Material Editor - no se puede hacer por código/texto):
  - Parámetro de textura `Heightmap` (R16F) + parámetro vectorial `HeightmapMinMax` (ya los escribe `SetChunkHeightmap` en `PlanetNaniteMesh.cpp:425-431`)
  - **Importante**: esto es una malla con Nanite habilitado. El WPO clásico tiene soporte limitado/con coste en Nanite - para desplazamiento grande de terreno hace falta usar el pin **Displacement** del material (Nanite Displacement, UE 5.3+) en vez de World Position Offset, y activar Displacement en el `FMeshNaniteSettings` de la malla base. Si se conecta al WPO clásico en vez de a Displacement, es fácil que compile y no se vea el efecto esperado - de ahí que no lo intentase a ciegas.
  - Una vez exista el material y `NaniteConfig.PlanetMaterial` apunte a él, conectar la elevación real de `RasterizedTectonics` al heightmap (hoy solo ruido, ver punto de arriba) es un cambio de código acotado.
