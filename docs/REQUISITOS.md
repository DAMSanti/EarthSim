...
**R2.9 — La propiedad se decide contra el marco propio de la placa, no contra el mundo de ayer.** Cada placa guarda su territorio (qué celdas son suyas) en un ráster en su propio sistema de referencia, igual que el material (R2.10), leído con la rotación **acumulada**. La pregunta «¿es tuyo este punto?» nunca se le hace al ráster del mundo del paso anterior — eso es remuestrear un campo categórico contra sí mismo cada paso, y pierde el movimiento sub-celda de forma irreversible en cualquier región que se mueva por debajo de ~1 celda/advección (documentado y medido: `Simu.Tectonics.StuckCellsNearEulerPole`, y su combinación con una frontera divergente vecina, que enmascara el rift y hace crecer territorio donde debería nacer corteza nueva).
*Redacción anterior (16-08-2026, insuficiente):* decidir contra el mundo actual con rotación incremental evita que la propiedad se pudra en ruido con muchas advecciones (el defecto de A10), pero no evita la degradación por redondeo de sub-celda en regiones lentas, porque sigue siendo remuestreo repetido de un campo categórico. El marco propio por placa, con escritura solo en el instante del evento tectónico (rift, colisión) — nunca por remuestreo masivo — no tiene ese problema: es el mismo principio que R2.10 ya usa para el material, aplicado también a la propiedad.
*Criterio:* el territorio de una celda de interior no cambia de valor sin que medie un evento de frontera (colisión o rift) que la afecte directamente — ninguna celda "olvida" su dueño por puro paso del tiempo, sin importar lo lenta que rote su placa.
