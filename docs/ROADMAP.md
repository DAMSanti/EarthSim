...
- [x] Fallas transformantes: fricción, esfuerzo acumulado y sismicidad de verdad (18-08-2026) — `FBoundarySegment::AccumulatedStrain` acumula deslizamiento tangencial solo cuando el régimen es transformante (se congela, no se resetea, si cambia a otro régimen); se libera en terremotos de tamaño fijo (`SeismicSlipThresholdMetres`, 3 m) vía Hanks-Kanamori (`Mw=(2/3)log10(M0)-6,07`), registrados en `SeismicHistory` y mostrados en el HUD. Ver [`ANEXO.md`](ANEXO.md#f1d-sismicidad-fricción-y-esfuerzo-acumulado-de-verdad-18-08-2026) para el problema de escala (Mw 10-12 al liberar todo de golpe) y el arreglo de fondo (acotar por construcción, no por umbral). Verificado con test: `Simu.Tectonics.TransformFaultSeismicity`
- [x] Registrar «tipo de frontera» como campo categórico en el visor (18-08-2026) — `BoundaryTypeData` es un espejo de la misma clasificación por segmento que ya decide orogenia/acreción/rift en `AdvectPlateField`, expuesto como campo `BoundaryType` (0=interior, 1=convergente, 2=divergente, 3=transformante). Verificado con test: `Simu.Tectonics.BoundaryTypeField`
...
- [x] **Muerte** (18-08-2026): área TOTAL de la placa (todos sus componentes, no solo los que fragmentan) por debajo de `MinPlateAreaCells` ⇒ se disuelve entera en sus vecinas, componente a componente, cada uno a quien más le rodee — incluso el que normalmente conservaría el ID. Vive dentro de `HandlePlateFragmentation()`, comparte el flood-fill. Ver [`ANEXO.md`](ANEXO.md#f1e-completo-muerte-sutura-polo-de-euler-propio-e-historial-18-08-2026)
...
- [x] Polo de Euler propio a la placa nueva (18-08-2026), derivado de la geometría de la ruptura: eje = centroide del hijo × dirección de alejamiento del centroide del padre (identidad del triple producto vectorial), misma magnitud de `AngularVelocity` que el padre. Antes copiaba EulerPole/AngularVelocity del padre tal cual; con cinemática fija eso nunca se corregía
- [x] **Sutura** (18-08-2026): dos placas con `FBoundarySegment::Age >= SutureAgeThresholdMa` y convergencia/deslizamiento tangencial casi nulos se fusionan — la de menos celdas se reasigna entera a la de más. Verificado en `Simu.Tectonics.F1EF1FLongRun`: 3 fusiones en 1000 Ma. Ver ANEXO.md
...
- [x] Gestionar el marco de material al morir y fusionar (18-08-2026): `ClearPlateFrame()` vacía `PlateMaterial`/`PlateTerritory` de la placa perdedora — nadie debe volver a leerlos
- [x] Historial de placas (18-08-2026): `FPlateLifecycleRecord`/`GetPlateHistory()` — quién nació de quién (fragmentación), quién asimiló a quién (isla huérfana), quién murió y quién se fusionó en quién, con el tiempo simulado de cada evento
...
- [x] Ciclo de vida completo verificado (18-08-2026, tras el arreglo del grosor): `Simu.Tectonics.F1EF1FLongRun` a resolución de producción, tierra emergida 24,9%→22,7% SIN colapso, 3 suturas, 1 nacimiento, 1585 asimilaciones. Nuevo `GetNumLivingPlates()` distingue placas vivas de tombstones -muerte/sutura nunca reducen `PlateSystem->GetNumPlates()` porque el PlateID es el índice y borrarlo en medio corrompería cada celda con un índice posterior-; en esa corrida, `GetNumPlates()` decía 9 pero solo 6 estaban realmente vivas. Test dedicado: `Simu.Tectonics.PlateLifecycle`, verifica conservación de celdas y que toda placa muerta/fusionada de verdad se quedó sin celdas, no solo que el evento se registró
